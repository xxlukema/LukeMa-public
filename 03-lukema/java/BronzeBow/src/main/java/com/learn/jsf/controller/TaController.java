...
public class TaController
    ...
    @Inject
    private DomainDataUtils domainDataUtils;
    ...
    public void init() {
        ...
        userLoginController.loginUserFromCookie();

        LOG.info("isUserLoggedIn: " + userLoginController.isUserLoggedIn());
    }
    ...
    protected void destroy() {
        ...
        LOG.info("TaController destroyed. id: " + id + " Created at: " + creationTime + ". Destroyed at: " + new Date() + ". Inactive time: " + hours + " hours "
                + minutes + " minutes");
    }

    protected void finalize() {
        LOG.info("TaController finalized. id: " + id + " Created at: " + creationTime + ". Finalized at: " + new Date());
    }
    ...
    public String drawChart() {
        ...
        try {
            ...
            List<Dividend> list = domainDataUtils.getDividendData(symbol);
    ...
    private List<HotListRow> getNewHotListRowList() {
        ...
        try {
            accessHotListList = accessService.retrieveAccessHotList(HotListSize);

            if (accessHotListList.size() == 0) {
                String[] symbols = { "amd", "lnkd", "aapl", "c", "ual", "cyd", "fro", "goog", "gs", "hov", "jpm", "lvs", "ms", "msft", "phm", "prft", "pba", "amzn",
                        "vale", "vlccf", "yhoo", "slb" };

                for (String sym : symbols) {
                    LOG.debug("Inserting " + sym);
                    accessService.updateAccessHotList(sym);
                }
            ...
            return new ArrayList<HotListRow>();
        }

        Collections.sort(accessHotListList, accessComparator);

        List<HotListRow> list = new ArrayList<HotListRow>();
    ...
    public List<HotListRow> getMyListRowList() {
        ...
        String stockList = userLoginController.getUser().getStockList();
        if (stockList != null) {
