...
{
   protected static final Logger LOG = Logger.getLogger(DomainDataUtils.class);

   protected static final String QQQ = "qqq";

   protected static Vector<RawData> QqqRawDataVector;

   private static final String[] INDEXES =
   { "^gspc", "^dji", "^ixic", "slv", "qqq", };

   public static Vector<Dividend> getDividendData(String symbol)
   {
      Vector<Dividend> list = new Vector<Dividend>();
   ...
   public static DataResponse doRequest(DataRequest dataRequest)
   {
      ...
      {
         String errorMessage = "The stock must have been trading for at least 30 days. Current data size: "
               + rawDataVector.size();
         LOG.error(errorMessage);
         dataResponse.setErrorMessage(errorMessage);
      ...
      if (!isIndex)
      {
         try
         {
            SpringServiceFacade.updateAccessHotList(symbol);
         }
      ...
      // 8. OBV PVT
      y0Position += ChartConstants.ChartSpacer + ChartConstants.IndicatorChartHeight;
      OnBalanceVolumeDomainData onBalanceVolumeDomainData = getOnBalanceVolumeDomainData(rawDataVector,
            y0Position);
      ...
      // 10. Chaikin
      y0Position += ChartConstants.ChartSpacer + ChartConstants.IndicatorChartHeight;
      ChaikinOscillatorDomainData chaikinOscillatorDomainData = getChaikinOscillatorDomainData(rawDataVector,
            y0Position);
   ...
   private static AccumDistLineDomainData getAccumDistLineDomainData(
         Vector<RawData> rawDataVector,
         int y0Position)
   {
   ...
   private static ChaikinOscillatorDomainData getChaikinOscillatorDomainData(
         Vector<RawData> rawDataVector,
         int y0Position)
   {
   ...
   private static OnBalanceVolumeDomainData getOnBalanceVolumeDomainData(
         Vector<RawData> rawDataVector,
         int y0Position)
   {
