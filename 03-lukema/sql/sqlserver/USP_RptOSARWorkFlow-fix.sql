...
DECLARE	@CalenderId INT
    ...
    WITH
        ...
        (
            ...
            AND dbo.fncCalIDToCal(ca.CalendarId) =@Calender
            GROUP BY
                statusPfs.PropertyID,
                statusPfs.CalendarID,
                statusPfs.UserID
    ...
    SELECT
        ...
        DATEDIFF(dd, CONVERT(VARCHAR(10), FL.IssueCloseDt, 101), CONVERT(VARCHAR(10),
        FL.IssueOpenDt, 101))                     AS TotalDaysOpen ,
    ...
    LEFT OUTER JOIN
        pfs_Pushed_StatusDate pD WITH(NOLOCK)
    ON
        pD.PropertyId = PFS.PropertyID
    AND pD.CalendarId = pfs.calendarId
    LEFT OUTER JOIN
        AppUserInfo am WITH(NOLOCK)
    ON
        am.UserID = I.AssetManager
    LEFT OUTER JOIN
        AppUserInfo pm WITH(NOLOCK)
    ON
        pm.userId = I.PSRTeamLead
    LEFT OUTER JOIN
        AppUserInfo reviewer WITH(NOLOCK)
    ON
        reviewer.UserID = Oa.ReviewerID
    LEFT OUTER JOIN
        AppUserInfo cd WITH(NOLOCK)
    ON
