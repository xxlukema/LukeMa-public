...
FROM
    ...
    (
        SELECT
            ...
            'YYYY')) , DECODE(Q1.Rule_Code,0,0,1,
            (PKG_CMSA_Reports_RemitSuppTab.SP_CMSA_FindASER_FNC (PIAdv.Investor_Id,PIAdv.Loan_ID,
            PIAdv.Advance_Dt)- PKG_CMSA_REPORTS_REMITTANCE.SP_CMSA_CalcLateASERPmt_FNC
            (AR.INVESTOR_ID, AR.LOAN_ID, PIADV.ADVANCE_DT, TO_CHAR(SYSDATE,'MM'),TO_CHAR(SYSDATE,
            'YYYY')) ), ROUND((NVL (PIAdv.Principal_Advance,0) + NVL(PIAdv.Net_Interest,0)) * NVL
