SELECT
    V_ADVANCES_ANALYSIS.INVESTOR_ID,
    V_ADVANCES_ANALYSIS.LOAN_ID,
    V_ADVANCES_ANALYSIS.LOAN_NAME,
    V_ADVANCES_ANALYSIS.LAST_PYMT_DT,
    V_ADVANCES_ANALYSIS.FST_ADVANCE_DT,
    V_ADVANCES_ANALYSIS.CURRENT_ADVANCE_DT,
    V_ADVANCES_ANALYSIS.CURBEGIN_SCHBAL,
    V_ADVANCES_ANALYSIS.TRANSFER_DT,
    V_ADVANCES_ANALYSIS.PRINCIPAL_ADVANCE_AMT,
    V_ADVANCES_ANALYSIS.NET_INTEREST_AMT,
    V_ADVANCES_ANALYSIS.TOTAL_PI_ADV_AMT,
    V_ADVANCES_ANALYSIS.CUMULATIVE_ASER_AMT,
    V_ADVANCES_ANALYSIS.TOTAL_SERV_ADV_AMT,
    V_ADVANCES_ANALYSIS.INTEREST_ON_ADV,
    V_ADVANCES_ANALYSIS.TOTAL_TI_ADV_AMT,
    V_ADVANCES_ANALYSIS.APPRAISAL_VALUE,
    V_ADVANCES_ANALYSIS.MOST_REC_APPRLDT,
    V_ADVANCES_ANALYSIS.ARA_AMT,
    V_ADVANCES_ANALYSIS.CURENDING_SCH_BAL,
    V_ADVANCES_ANALYSIS.ASERAMT
FROM
    (
        SELECT
            ...
            T37.FST_ADVANCE_DT,
            PKG_CMSA_Functions.Sp_Cmsa_Repdeterdt_Fnc(POOL.INVESTOR_ID,TO_CHAR(SYSDATE,'MM'),
            TO_CHAR (SYSDATE,'YYYY'))                                CURRENT_ADVANCE_DT,
            ...
            DECODE(T38.PRINCIPAL_ADVANCE_AMT,0,NULL,T38.PRINCIPAL_ADVANCE_AMT)
            ...
            PKG_CMSA_Reports_Remittance.SP_CMSA_SERVOUTSTAND_FNC(POOL.INVESTOR_ID,T21.LOAN_ID,
            TO_CHAR (SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY')) "TOTAL_SERV_ADV_AMT",
            T40.INTEREST_ON_ADV,
            PKG_CMSA_Reports_Remittance.SP_CMSA_TIOUTSTAND_FNC(POOL.INVESTOR_ID,T21.LOAN_ID,TO_CHAR
            (SYSDATE ,'MM'),TO_CHAR(SYSDATE,'YYYY'))                      "TOTAL_TI_ADV_AMT", --L38
        ...
        FROM
            ...
            (
                SELECT
                    LP.LOAN_ID,
                    DECODE(MIN(DECODE(APPRAISAL_DT,NULL,0)),0,NULL,MAX(APPRAISAL_DT))
                    "MOST_REC_APPRLDT"
                FROM
                ...
                AND PA.APPRAISAL_DT <= PKG_CMSA_Functions.Sp_Cmsa_Repdeterdt_Fnc(P.INVESTOR_ID,
                ...
                AND LP.PROPERTY_ID NOT IN
                    (
                        ...
                        WHERE
                            LOAN_ID=P.LOAN_ID
                        AND EFFECTIVE_DT <=PKG_CMSA_Functions.Sp_Cmsa_Repdeterdt_Fnc(P.INVESTOR_ID,
                            TO_CHAR (SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY')))
                GROUP BY
                    LP.LOAN_ID) T19,
            --L75
            ...
            (
                SELECT
                    AR.INVESTOR_ID,
                    AR.LOAN_ID,
                    DECODE(PIAdv.Advance_Dt,NULL,0-
                    PKG_CMSA_REPORTS_REMITTANCE.SP_CMSA_CalcLateASERPmt_FNC (AR.INVESTOR_ID,
                    AR.LOAN_ID, PIR.RECOVERY_DT, TO_CHAR(SYSDATE,'MM'),TO_CHAR(SYSDATE, 'YYYY')) ,
                    ...
                    PKG_CMSA_REPORTS_REMITTANCE.SP_CMSA_CalcLateASERPmt_FNC(AR.INVESTOR_ID,
                    AR.LOAN_ID, PIADV.ADVANCE_DT, TO_CHAR(SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY')) ),
                    ROUND((NVL (PIAdv.Principal_Advance,0) + NVL(PIAdv.Net_Interest,0)) * NVL
                    (PIAdv.ASER_Percnt,0) - PKG_CMSA_REPORTS_REMITTANCE.SP_CMSA_CalcLateASERPmt_FNC
                    (AR.INVESTOR_ID, AR.LOAN_ID, PIADV.ADVANCE_DT, TO_CHAR(SYSDATE,'MM'),TO_CHAR
                    (SYSDATE,'YYYY')) ,2))) AS ASERAMT,
                    PKG_CMSA_Reports_Remittance.SP_CMSA_CumASERBalance_FNC(AR.INVESTOR_ID,
                    AR.LOAN_ID, TO_CHAR(SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY')) CumASER
                FROM
                    T_CMSA_LOAN_ARA AR,
                    ...
                    (
                        ...
                        FROM
                            ...
                            --T_CMSA_APPRAISAL_REDUCTION AR
                        WHERE
                            AR.ARA_DT <= PKG_CMSA_Functions.Sp_Cmsa_Repdeterdt_Fnc(AR.INVESTOR_ID,
                            TO_CHAR (SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY'))
                        GROUP BY
                            INVESTOR_ID,
                            LOAN_ID) Q,
                ...
                AND PKG_CMSA_Reports_Remittance.SP_CMSA_Status_BeginDt_FNC(AR.INVESTOR_ID,
                    AR.LOAN_ID, TO_CHAR(SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY')) IS NOT NULL
                ...
                AND PIAdv.Advance_Dt =
                    (
                        ...
                        AND PI.Advance_Dt <= PKG_CMSA_Functions.Sp_Cmsa_Trusteeremitdt_Fnc
                            (PI.INVESTOR_ID, TO_CHAR(SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY')))
            ...
            (
                SELECT
                    ...
                    ,
                    PKG_CMSA_Reports_Remittance.SP_CMSA_PIOUTSTAND_FNC(q1.INVESTOR_ID,q1.LOAN_ID,
                    TO_CHAR (SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY')) TOTAL_PI_ADV_AMT
                FROM
                    (
                        SELECT
                            A.INVESTOR_ID,
                        ...
                        FROM
                            T_CMSA_PI_ADVANCE A
                        WHERE
                            A.ADVANCE_DT BETWEEN
                            PKG_CMSA_REPORTS_REMITTANCE.SP_CMSA_STATUS_BEGINDT_FNC (A.INVESTOR_ID,
                            A.LOAN_ID,TO_CHAR(SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY')) AND
                            PKG_CMSA_FUNCTIONS.SP_CMSA_TrusteeRemitDt_FNC(A.INVESTOR_ID,TO_CHAR
                    ...
                    (
                        SELECT
                            ...
                            SUM( ROUND((NVL(Principal_Recovery,0) + NVL(Interest_Recovery,0)) - (
                            ((NVL (Principal_Recovery,0) + NVL(Interest_Recovery,0)) *
                            (A.Principal_Advance + A.Interest_Advance - A.Actual_PI)) /
                            (A.Principal_Advance + A.Interest_Advance) ),2) ) - SUM
                            (R.PRINCIPAL_RECOVERY) I_REC
                        FROM
                        ...
                        AND R.Recovery_Dt BETWEEN
                            PKG_CMSA_REPORTS_REMITTANCE.SP_CMSA_STATUS_BEGINDT_FNC (R.INVESTOR_ID,
                            R.LOAN_ID,TO_CHAR(SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY')) AND
                            PKG_CMSA_FUNCTIONS.SP_CMSA_REPDETERDT_FNC(R.INVESTOR_ID,TO_CHAR(SYSDATE
            ...
            (
                ...
                FROM
                    (
                        SELECT
                            P.INVESTOR_ID,
                            SUM(PKG_CMSA_Functions.SP_CMSA_EndSchBal_FNC(P.INVESTOR_ID,LOAN_ID,
                            TO_CHAR (SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY'),CAL.REPORT_DETER_DT))
                            "CURENDING_SCH_BAL"
                        FROM
                            T_CMSA_POOL P,
            ...
            (
                SELECT
                    ...
                    CAL.REPORT_DETER_DT, TO_CHAR(SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY'))
                    INTEREST_ON_ADV
                FROM
                    T_CMSA_POOL P,
                    T_CMSA_DISTRIBUTION_CALENDAR CAL
