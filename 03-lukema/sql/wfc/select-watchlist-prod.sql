...
SELECT
    ...
    T2.CITY,
    T2.STATE,
    PKG_CMSA_Functions.Sp_Cmsa_Datefrmt_Fnc(PKG_CMSA_Functions.SP_CMSA_DeterDt_FNC(769,
    PKG_CMSA_Functions.Sp_Cmsa_Datefrmt_Fnc(W.EFFECTIVE_DT))) "EFFECTIVE_DT",
    PKG_CMSA_Functions.SP_CMSA_Endschbal_FNC(P.INVESTOR_ID,W.LOAN_ID,11,2012,NULL)
                                                           "END_SCH_BAL",
    ...
    ,W.LOAN_ID,11,2012)) "MATURITY_DT",
    PKG_CMSA_FUNCTIONS.SP_CMSA_CROSSCOLLNCFLN_FNC(W.LOAN_ID,1,'AN',769,11,
    2012)                                                   "FISCAL_YR_DSCRNCF",
    ...
    Sp_Cmsa_Loan_Refcd_Criteria(W.LOAN_ID,20121113)                                  "REFCD_CRITERIA"
    ,
    W.COMMENTS
FROM
    (
    ...
    (
        ...
        FROM
            ...
            (
                ...
                AND Feed_Dt BETWEEN v_dLowerBndDt AND 20121113
...
AND NOT EXISTS
    (
        ...
        WHERE
            LOAN_ID=P.LOAN_ID
        AND TRANSFER_DT<20121113
        AND (
                RETURN_DT IS NULL
            OR  RETURN_DT>20121113) )
AND W.EFFECTIVE_DT<=20121113
AND (
...
AND W.LOAN_ID NOT IN
    (
        ...
        AND DEFEASANCE_CD = 'F' )
AND P.INVESTOR_ID=769
AND Sp_Cmsa_Loan_Refcd_Criteria(W.LOAN_ID,20121113) IS NOT NULL;
