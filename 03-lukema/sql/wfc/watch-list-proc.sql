...
OR
REPLACE
PROCEDURE "CMSA"."SP_CMSA_REPORTS_WATCHLIST" (in_nInvestorId IN NUMBER,
                                              in_nRepmonth IN NUMBER,
                                              in_nRepyear IN NUMBER,
                                              out_dAsOfDt OUT VARCHAR,
                                              out_nStatusCode OUT NUMBER,
                                              out_cvGeneric IN OUT PKG_CMSA_CommDefi.GenCurTyp,
                                              out_cvGenericTot IN OUT PKG_CMSA_CommDefi.GenCurTyp)
IS
    v_AsOfDate DATE;
v_dDistriDt DATE;
v_dLowerBndDt DATE;
...
SELECT-- Added by RAMKUMAR MADDELA 7/18/2007 (Transactionid, groupid, loanid)
    INV.TRANSACTION_ID,
    L.GROUP_ID,
    L.LOAN_ID,
    ...
    PKG_CMSA_Functions.Sp_Cmsa_Datefrmt_Fnc(W.EFFECTIVE_DT))) "EFFECTIVE_DT", --New Column L105
