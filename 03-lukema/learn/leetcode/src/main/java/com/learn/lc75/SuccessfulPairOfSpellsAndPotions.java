...
public class SuccessfulPairOfSpellsAndPotions {
    ...
    public static void main(String[] args) {
        ...
        var ret = successfulPairOfSpellsAndPotions.successfulPairs(spells, potions, success);
        log.debug("Successful Pairs of Spells and Potions: {}", () -> ret);
        Assertions.assertArrayEquals(expected, ret);
        log.debug("Successful Pairs of Spells and Potions: {} OK", () -> "successfulPairs");
    ...
    public int[] successfulPairs(int[] spells, int[] potions, long success) {
        ...
        for (int i = 0; i < spells.length; i++) {
            int multiplier = spells[i];
            int count = 0;
            for (int pot : potions) {
                if (multiplier * pot >= success) {
                    count++;
                }
            }
