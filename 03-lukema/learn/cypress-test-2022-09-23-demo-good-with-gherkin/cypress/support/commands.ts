...
Cypress.Commands.add('kcLogout', () => {
  /**
   * Trick 1: Use of `Cypress.env('kcRoot')`
   *
   * `Cypress.env('kcRoot')` is defined in `cypress.config.ts`
   */
  const kcRoot = Cypress.env('kcRoot');
  const kcRealm = Cypress.env('kcRealm');
  /**
   * Trick 2: Use of `Cypress.config().baseUrl`
   *
   * `Cypress.config().baseUrl` is defined in `cypress.config.ts`
   */
