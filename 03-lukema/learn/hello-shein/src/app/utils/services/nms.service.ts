...
export class NmsService {
  ...
  removeDuplicatedWhiteSpaces(input: string | null | undefined): string | null | undefined {
    if (input) {
  ...
  getDispalyName(username: string) {
  ...
  handleError = (error: HttpErrorResponse) => {
    console.error('=== 3 === NmsService handleError HttpClient', 'error', error);

    if (error.error.message) {
      console.error('=== 3 === NmsService handleError HttpClient', 'error', error);
    }

    return throwError(() => 'Error-333333: ' + error.error.message);
  };
  ...
  /**
   * 'base64url': ERROR TypeError: Unknown encoding: base64url
   *
  ...
  */
  ...
  toBase64Urlsafe(str: string): string {
    if (str) {
      str = URLSafeBase64.encode(Buffer.from(str));
    }
    return str;
  }
  ...
  private errorUrl = '/home/error/error';
